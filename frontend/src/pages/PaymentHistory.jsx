...
const PaymentHistory = () => {
  ...
  return (
    <div className="page-container mobile-links-page" style={{ maxWidth: 900 }}>
      <h2 style={{ textAlign: 'center', marginBottom: 32, color: '#1f2937', letterSpacing: 1 }}>Payment History</h2>
      <div style={{ display: 'flex', gap: 12, justifyContent: 'center', marginBottom: 12 }}>
        {tenantOptions.length > 0 && (
          <select value={filterTenant} onChange={e => setFilterTenant(e.target.value)} style={{ padding: 10, borderRadius: 6, border: '1px solid #d4a017' }}>
            <option value="">Tenant (All)</option>
            {tenantOptions.map(t => (<option key={t.value} value={t.value}>{t.label}</option>))}
          </select>
        )}
        <input type="month" value={filterDate} onChange={e => setFilterDate(e.target.value)} style={{ padding: 10, borderRadius: 6, border: '1px solid #d4a017' }} />
        <button onClick={() => { setFilterDate(""); setFilterTenant(""); }} style={{ padding: 10, borderRadius: 6, background: '#fff3cf', border: '1px solid #d4a017', color: '#1f2937', fontWeight: 600 }}>Clear Filters</button>
      </div>
      <div style={{ overflowX: 'auto', borderRadius: 8, boxShadow: '0 2px 12px rgba(0,0,0,0.07)' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 16, background: '#fff' }}>
          <thead style={{ background: '#fff3cf' }}>
            <tr>
              <th style={{ padding: '12px 8px', fontWeight: 600, color: '#1f2937', borderBottom: '2px solid #f1deab' }}>Date & Time</th>
              ...
              <th style={{ padding: '12px 8px', fontWeight: 600, color: '#1f2937', borderBottom: '2px solid #f1deab' }}>Transaction ID</th>
              <th style={{ padding: '12px 8px', fontWeight: 600, color: '#1f2937', borderBottom: '2px solid #f1deab' }}>Status</th>
              <th style={{ padding: '12px 8px', fontWeight: 600, color: '#1f2937', borderBottom: '2px solid #f1deab' }}>Details</th>
            </tr>
          </thead>
          <tbody>
            {filtered.length === 0 ? (
              ...
              >
                ...
                <td style={{ padding: '10px 8px', fontWeight: 600, color: '#b45309' }}>₹{item.amount}</td>
      ...
      {selectedInvoice && (
        <div style={{
          ...
          <div style={{
            ...
            <p><strong>Amount:</strong> <span style={{ color: '#b45309', fontWeight: 600 }}>₹{selectedInvoice.amount}</span></p>
