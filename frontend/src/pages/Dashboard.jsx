          ...
          <div className="dash-content">
              ...
              {ownerLoading ? (
                ...
                <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start' }}>
                  <div style={{ minWidth: 160 }}>
                    <div style={{ width: 120, height: 120, borderRadius: '50%', background: '#fff3cf', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 36, color: '#b45309', overflow: 'hidden' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    {ownerEditMode ? (
                      ...
                      <div>
                        ...
                        <div style={{ marginTop: 12 }}>
                          <button onClick={() => setOwnerEditMode(true)} style={{ padding: '8px 12px', borderRadius: 6, background: '#d4a017', color: '#1f2937', border: '1px solid #c28e14', fontWeight: 600 }}>Edit Profile</button>
                        </div>
                      </div>
                    )}
                  </div>
            ...
            <div className="dash-section" style={{ marginTop: "2rem" }}>
              ...
              <div className="dash-quick-actions">
                    <div className="dash-card dash-quick-card" onClick={() => nav('/all-tenants')}>
                  <div className="dash-quick-card__inner">
                    <div className="dash-quick-card__icon">👥</div>
                    <div>
                      <div className="dash-quick-card__value">{propsList.reduce((acc, p) => acc + (p.units?.filter(u => u.tenant)?.length || 0), 0)}</div>
                      <div className="dash-quick-card__label">Total Tenants</div>
                    </div>
                  </div>
                </div>

                    <div className="dash-card dash-quick-card" onClick={() => nav('/invoice')}>
                  <div className="dash-quick-card__inner">
                    <div className="dash-quick-card__icon">📄</div>
                    <div>
                      <div className="dash-quick-card__value">Invoices</div>
                      <div className="dash-quick-card__label">Manage invoices</div>
                    </div>
                  </div>
                </div>

                    <div className="dash-card dash-quick-card" onClick={() => nav('/maintenance')}>
                  <div className="dash-quick-card__inner">
                    <div className="dash-quick-card__icon">🛠️</div>
                    <div>
                      <div className="dash-quick-card__value">Maintenance</div>
                      <div className="dash-quick-card__label">View requests</div>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="dash-content">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
              <div>
                ...
                <p style={{ color: '#64748b', marginTop: 6 }}>{myUnit ? `${myUnit.property?.name || myUnit.propertyName || ''} • Unit ${myUnit.number || myUnit.unitNumber || ''}` : 'No unit assigned'}</p>
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={() => nav('/invoice')} style={{ padding: '8px 14px', borderRadius: 6, background: '#d4a017', color: '#1f2937', border: '1px solid #c28e14', fontWeight: 600 }}>View Invoices</button>
                <button onClick={() => nav('/payment-history')} style={{ padding: '8px 14px', borderRadius: 6, background: '#fff', color: '#b45309', border: '1px solid #d4a017', fontWeight: 600 }}>Payment History</button>
              </div>
            </div>
                  ...
                  {invoices.slice(0,3).map(inv => (
                    <div key={inv._id} className="dash-card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      <div style={{ textAlign: 'right' }}>
                        <div style={{ fontWeight: 700, color: '#b45309' }}>₹{inv.rent || inv.amount}</div>
                        {inv.status !== 'paid' ? (
                          <button onClick={() => nav('/invoice')} style={{ marginTop: 6, padding: '6px 12px', borderRadius: 6, background: '#d4a017', color: '#1f2937', border: '1px solid #c28e14', fontWeight: 600 }}>Pay Now</button>
                        ) : (
