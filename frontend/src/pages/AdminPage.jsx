...
export default function AdminPage() {
  ...
  const handleApprovePayment = async (paymentId) => {
    try {
      ...
      const res = await api.post(`/subscription/admin/purchases/${paymentId}/approve`);
      setPayments((prev) =>
        prev.map((p) => (p._id === paymentId ? { ...p, ...res.data.subscription } : p)),
      );
  ...
  const handleRejectPayment = async (paymentId) => {
    ...
    try {
      setPaymentAction(paymentId);
      const res = await api.post(`/subscription/admin/purchases/${paymentId}/reject`, {
        reviewNote: note || "",
      });
      setPayments((prev) =>
        prev.map((p) => (p._id === paymentId ? { ...p, ...res.data.subscription } : p)),
    ...
    <div className="admin-page-shell">
      <div className="admin-page-head">
        ...
        <div className="admin-head-actions">
          <button onClick={() => nav("/admin/blogs")} className="btn-secondary">Manage Blogs</button>
          <button onClick={() => nav("/admin/blogs/new")}>Create Blog</button>
        </div>
      </div>
        ...
        <section className="admin-card admin-card--wide">
              ...
              <tbody>
                {pagedPayments.map((pay, idx) => (
                  <tr key={pay._id}>
                    ...
                    <td>
                      {pay.status === "approved" || pay.status === "active" ? (
                    ...
                    <td>
                      <div style={{ display: "flex", gap: 8 }}>
                        <button
                          ...
                          disabled={paymentAction === pay._id}
                        >
                          Approve
                        </button>
                        <button
          ...
          {payments.length > pageSize && (
            <div style={{ display: "flex", justifyContent: "center", gap: 10, marginTop: 12 }}>
              <button type="button" className="btn-secondary" onClick={() => setPageIndex((p) => Math.max(0, p - 1))} disabled={pageIndex === 0}>
                Prev
              </button>
              <div style={{ alignSelf: "center" }}>
                Page {pageIndex + 1} of {totalPages}
              </div>
              <button type="button" className="btn-secondary" onClick={() => setPageIndex((p) => Math.min(totalPages - 1, p + 1))} disabled={pageIndex >= totalPages - 1}>
                Next
              </button>
            </div>
          )}
