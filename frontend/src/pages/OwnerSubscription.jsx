...
export default function OwnerSubscription() {
  ...
  const openPurchaseDialog = async (plan) => {
    ...
    try {
      const [infoRes, previewRes] = await Promise.all([
        api.get("/subscription/payment-info"),
        api.get(`/subscription/purchase-preview?planCode=${encodeURIComponent(plan.code)}`),
      ]);
      setPaymentInfo(infoRes.data?.paymentInfo || null);
      setPurchasePreview(previewRes.data?.pricing || null);
    } catch (_err) {
      setPaymentInfo(null);
      setPurchasePreview(null);
    }
  ...
  const handlePay = async () => {
    ...
    const finalPayable = Number(purchasePreview?.finalPayableAmount ?? selectedPlan.price ?? 0);
    if (finalPayable > 0 && !txnId.trim()) {
        ...
        <>
          {status?.hasActivePackage && status?.activeSubscription ? (
            <div className="success-msg" style={{ marginBottom: 14 }}>
              <div>
                Current Plan: <strong>{status.activeSubscription.planName}</strong>
              </div>
              <div>
                Expires in: <strong>{Math.max(0, Number(status.activeSubscriptionRemainingDays || 0))} day(s)</strong>
              </div>
              <div>
                Valid till: <strong>{new Date(status.activeSubscription.endsAt).toLocaleDateString()}</strong>
              </div>
            </div>
            ...
            {plans.map((p) => (
              <div key={p.code} className="dash-card" style={{ cursor: "default" }}>
                ...
                <button
                  ...
                  disabled={
                    !canSelfPurchase ||
                    (status?.hasActivePackage &&
                      (status?.activeSubscription?.planCode === p.code ||
                        status?.activeSubscription?.planName === p.name))
                  }
                  style={{
                    background: "#d4a017",
          ...
          <div style={dialogStyle} onClick={(e) => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "flex-start", gap: 10, alignItems: "center" }}>
              <button
                type="button"
                onClick={closeDialog}
                style={{ border: "none", background: "transparent", fontSize: 24, lineHeight: "22px", cursor: "pointer", width: "fit-content", padding: 0 }}
                aria-label="Close"
              >
                x
              </button>
            ...
            <div style={{ marginTop: 10 }}>
              <div style={{ marginBottom: 12, padding: 10, border: "1px solid #e5e7eb", borderRadius: 8, background: "#f8fafc" }}>
                <h3 style={{ marginBottom: 8 }}>Price Breakdown</h3>
                <p style={{ marginBottom: 6 }}>
                  New Plan Price: <strong>Rs {purchasePreview?.newPlanPrice ?? selectedPlan.price}</strong>
                </p>
                <p style={{ marginBottom: 6 }}>
                  Credit from Current Plan ({purchasePreview?.remainingDays || 0} unused days): <strong>- Rs {purchasePreview?.remainingValue || 0}</strong>
                </p>
                <p style={{ marginBottom: 0, borderTop: "1px dashed #cbd5e1", paddingTop: 8 }}>
                  Final Payable Amount: <strong>Rs {purchasePreview?.finalPayableAmount ?? selectedPlan.price}</strong>
                </p>
              </div>

              <p style={{ marginBottom: 8 }}>
                Amount to pay: <strong>Rs {purchasePreview?.finalPayableAmount ?? selectedPlan.price}</strong>
              </p>
              ...
              <div style={{ marginBottom: 10 }}>
                ...
                <input
                  ...
                  placeholder={Number(purchasePreview?.finalPayableAmount || 0) > 0 ? "Enter transaction ID" : "Optional when payable amount is 0"}
                />
              </div>
              ...
              <div style={{ display: "flex", justifyContent: "flex-end" }}>
                <button
                  ...
                  onClick={handlePay}
                  disabled={
                    buying === selectedPlan.code ||
                    (!txnId.trim() && Number(purchasePreview?.finalPayableAmount || selectedPlan.price || 0) > 0) ||
                    !canSelfPurchase ||
                    !hasAdminPaymentInfo
                  }
                >
