              ...
              <div style={{ background: '#fff', borderRadius: 12, padding: 20, width: 720, maxWidth: '98%', maxHeight: '90vh', overflowY: 'auto', boxSizing: 'border-box', position: 'relative' }}>
                ...
                <div style={{ marginTop:12 }}>
                  ...
                  {createAmenities.map((a, idx) => (
                    <div key={idx} style={{ marginTop:6, borderRadius:6, padding:8, background:'#fafafa', boxSizing:'border-box' }}>
                      <div style={{ marginBottom: 6 }}>Amenity {idx + 1}</div>
                      <div style={{ display:'flex', gap:8, alignItems:'center' }}>
                        <input placeholder="Name" value={a.name} onChange={e => setCreateAmenities(prev => prev.map((it,i) => i===idx?{...it,name:e.target.value}:it))} style={{ flex:2, minWidth:220, padding:8, borderRadius:6 }} />
                        ...
                        <button onClick={() => setCreateAmenities(prev => prev.filter((_,i)=>i!==idx))} style={{ background:'#ef4444', color:'#fff', border:'none', padding:'6px 8px', borderRadius:6, width:120 }}>Remove</button>
                      </div>
                      <div style={{ marginTop:8 }}>
                        <input placeholder="Amenity description (optional)" value={a.description || ''} onChange={e => setCreateAmenities(prev => prev.map((it,i) => i===idx?{...it,description:e.target.value}:it))} style={{ width:'100%', padding:8, borderRadius:6, boxSizing:'border-box' }} />
                      </div>
                    </div>
                  ))}
                  <div style={{ marginTop:8 }}>
                    <button onClick={() => setCreateAmenities(prev => ([...prev, { name:'', cost:0, description: '' }]))} style={{ background:'#0ea5e9', color:'#fff', border:'none', padding:'8px 12px', borderRadius:6 }}>Add Amenity</button>
                  </div>
