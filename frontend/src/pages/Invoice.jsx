                ...
                {filteredInvoices.map((inv, idx) => (
                  <tr key={inv._id} style={{ background: idx % 2 === 0 ? '#f1f5f9' : '#fff', borderBottom: '1px solid #e0e0e0', cursor: 'pointer' }} onClick={() => setSelectedInvoice(inv)}>
                    ...
                    <td style={{ padding: '10px 8px', display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'nowrap' }} onClick={(e) => e.stopPropagation()}>
                      ...
                      <button style={{ padding: '4px 12px', borderRadius: 4, background: '#0e7490', color: '#fff', border: 'none', fontWeight: 600, cursor: 'pointer', width: 'fit-content', flexShrink: 0 }} onClick={(e) => { e.stopPropagation(); handleActionUpdate(inv._id); }} disabled={actionLoading === inv._id || (localActions[inv._id] === (inv.action || 'pending'))}>Update</button>
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            return (
              <div key={inv._id} style={{ background: '#fff', borderRadius: 14, boxShadow: '0 2px 16px #0001', padding: 24, display: 'flex', flexDirection: 'column', gap: 8, borderLeft: inv.status === 'paid' ? '6px solid #22c55e' : '6px solid #f59e42', position: 'relative' }}>
                ...
                <div style={{ fontSize: 15, marginTop: 2 }}>
                  Verification:{" "}
                  <span style={{ color: (inv.action || "pending") === "verified" ? "#22c55e" : "#f59e42", fontWeight: 600 }}>
                    {(inv.action || "pending") === "verified" ? "Verified" : "Pending"}
                  </span>
                </div>
                {inv.status === 'paid' ? (
