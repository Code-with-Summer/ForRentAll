...
export default function Invoice() {
  ...
  const renderSourceBadge = (inv) => {
    ...
    let bg = '#f1f5f9';
    let color = '#475569';
    if (origin === 'owner') { text = 'Owner'; bg = '#fff3cf'; color = '#b45309'; }
  ...
  const computeInvoiceAmount = (inv) => {
    ...
    // If rent is stored separately, total = rent + all amenity costs
    // Otherwise fall back to stored amount, then rent field fallbacks
    const amt = inv.rent != null
      ? Number(inv.rent) + amenitiesTotal
      : (typeof inv.amount !== 'undefined' && inv.amount !== null && String(inv.amount) !== '')
        ? Number(inv.amount)
        : amenitiesTotal;
  ...
  if (role === 'owner') {
    ...
    return (
      <div className="page-container mobile-links-page" style={{ padding: 24, maxWidth: 1200 }}>
        <div className="invoice-card" style={{ background: '#fff', borderRadius: 12, boxShadow: '0 2px 16px #0001', padding: 32 }}>
          <h2 className="invoice-title" style={{ textAlign: 'center', color: '#1f2937', marginBottom: 28 }}>All Tenant Invoices</h2>

          {/* Filters */}
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 14, marginBottom: 18, justifyContent: 'center' }}>
            <input type="month" value={filterDate} onChange={e => setFilterDate(e.target.value)} style={{ padding: 6, borderRadius: 6, border: '1px solid #f1deab' }} />
            <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} style={{ padding: 6, borderRadius: 6, border: '1px solid #f1deab' }}>
              <option value="">Status (All)</option>
              <option value="due">Due</option>
              <option value="paid">Paid</option>
            </select>
            <select value={filterAction} onChange={e => setFilterAction(e.target.value)} style={{ padding: 6, borderRadius: 6, border: '1px solid #f1deab' }}>
              <option value="">Action (All)</option>
              <option value="pending">Pending</option>
              <option value="verified">Verified</option>
            </select>
            <select value={filterProperty} onChange={e => setFilterProperty(e.target.value)} style={{ padding: 6, borderRadius: 6, border: '1px solid #f1deab' }}>
              <option value="">Property (All)</option>
              {propertyOptions.map(p => (<option key={p} value={p}>{p}</option>))}
            </select>
            <select value={filterUnit} onChange={e => setFilterUnit(e.target.value)} style={{ padding: 6, borderRadius: 6, border: '1px solid #f1deab' }}>
              <option value="">Unit (All)</option>
              {unitOptions.map(u => (<option key={u} value={u}>{u}</option>))}
            </select>
            <select value={filterTenant} onChange={e => setFilterTenant(e.target.value)} style={{ padding: 6, borderRadius: 6, border: '1px solid #f1deab' }}>
              <option value="">Tenant (All)</option>
              {tenantOptions.map(t => (<option key={t} value={t}>{t}</option>))}
            </select>
            <button onClick={() => { setFilterDate(""); setFilterStatus(""); setFilterAction(""); setFilterProperty(""); setFilterUnit(""); setFilterTenant(""); }} style={{ padding: 6, borderRadius: 6, background: '#fff3cf', border: '1px solid #f1deab', color: '#b45309', fontWeight: 600 }}>Clear</button>
          </div>

          {/* Export + Create */}
          <div style={{ display: 'flex', gap: 12, marginBottom: 18, justifyContent: 'center' }}>
            <button onClick={() => exportInvoicesToPDF(filteredInvoices)} style={{ background: '#d4a017', color: '#1f2937', border: '1px solid #c28e14', borderRadius: 6, padding: '8px 18px', fontWeight: 600 }}>Export PDF</button>
            <button onClick={() => exportInvoicesToExcel(filteredInvoices)} style={{ background: '#d4a017', color: '#1f2937', border: '1px solid #c28e14', borderRadius: 6, padding: '8px 18px', fontWeight: 600 }}>Export Excel</button>
          ...
          <div style={{ overflowX: 'auto', borderRadius: 8, boxShadow: '0 2px 12px rgba(0,0,0,0.07)' }}>
            <table className="invoice-table" style={{ width: '100%', borderCollapse: 'collapse', fontSize: 16, background: '#fff' }}>
              <thead style={{ background: '#fff3cf' }}>
                <tr>
                  <th style={{ padding: '12px 8px', fontWeight: 600, color: '#1f2937', borderBottom: '2px solid #f1deab' }}>Tenant</th>
                  <th style={{ padding: '12px 8px', fontWeight: 600, color: '#1f2937', borderBottom: '2px solid #f1deab' }}>Unit</th>
                  <th style={{ padding: '12px 8px', fontWeight: 600, color: '#1f2937', borderBottom: '2px solid #f1deab' }}>Month</th>
                  <th style={{ padding: '12px 8px', fontWeight: 600, color: '#1f2937', borderBottom: '2px solid #f1deab' }}>Amount</th>
                  <th style={{ padding: '12px 8px', fontWeight: 600, color: '#1f2937', borderBottom: '2px solid #f1deab' }}>Status</th>
                  <th style={{ padding: '12px 8px', fontWeight: 600, color: '#1f2937', borderBottom: '2px solid #f1deab' }}>Action</th>
                </tr>
              </thead>
              <tbody>
                {filteredInvoices.map((inv, idx) => (
                  <tr key={inv._id} style={{ background: idx % 2 === 0 ? '#f1f5f9' : '#fff', borderBottom: '1px solid #e0e0e0', cursor: 'pointer' }} onClick={() => setSelectedInvoice(inv)}>
                    ...
                    <td style={{ padding: '10px 8px', fontWeight: 500, color: '#b45309' }}>
                      {(() => {
                    ...
                    <td style={{ padding: '10px 8px', display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'nowrap' }} onClick={(e) => e.stopPropagation()}>
                      ...
                      <select value={localActions[inv._id] || 'pending'} onChange={e => handleActionDropdownChange(inv._id, e.target.value)} onClick={(e) => e.stopPropagation()} style={{ padding: '4px 8px', borderRadius: 4, border: '1px solid #ccc', background: '#fff', fontWeight: 500, width: 'fit-content', flexShrink: 0 }}>
                        <option value="pending">Pending</option>
                        <option value="verified">Verified</option>
                      </select>
                      <button style={{ padding: '4px 12px', borderRadius: 4, background: '#d4a017', color: '#1f2937', border: '1px solid #c28e14', fontWeight: 600, cursor: 'pointer', width: 'fit-content', flexShrink: 0 }} onClick={(e) => { e.stopPropagation(); handleActionUpdate(inv._id); }} disabled={actionLoading === inv._id || (localActions[inv._id] === (inv.action || 'pending'))}>Update</button>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          {selectedInvoice && (
            <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(0,0,0,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, overflowY: 'auto', padding: 20, boxSizing: 'border-box' }}>
              <div style={{ background: '#fff', padding: 24, borderRadius: 12, minWidth: 320, maxWidth: 560, width: '100%', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 4px 32px rgba(0,0,0,0.18)', position: 'relative', animation: 'fadeIn 0.2s', boxSizing: 'border-box' }}>
                ...
                <h3 style={{ color: '#1f2937', marginBottom: 18 }}>Invoice Details</h3>
                ...
                <p><strong>Amount:</strong> <span style={{ color: '#b45309', fontWeight: 600 }}>₹{selectedInvoice.amount || selectedInvoice.rent || computeInvoiceAmount(selectedInvoice).amt}</span></p>
            ...
            <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(0,0,0,0.25)', zIndex: 1200, display: 'flex', alignItems: 'center', justifyContent: 'center', overflowY: 'auto', padding: 20, boxSizing: 'border-box' }}>
              <div style={{ background: '#fff', borderRadius: 12, padding: 20, width: 720, maxWidth: '98%', maxHeight: '90vh', overflowY: 'auto', boxSizing: 'border-box', position: 'relative' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <button onClick={() => setShowCreateModal(false)} style={{ background: 'none', border: 'none', width: 26, height: 26, padding: 0, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, cursor: 'pointer', color: '#888', fontWeight: 700 }} aria-label="Close">×</button>
                </div>
                ...
                <div style={{ marginTop:12 }}>
                  ...
                  <div style={{ marginTop:8 }}>
                    <button onClick={() => setCreateAmenities(prev => ([...prev, { id: Date.now() + Math.random(), name:'', cost:'', description: '' }]))} style={{ background:'#d4a017', color:'#1f2937', border:'1px solid #c28e14', padding:'8px 12px', borderRadius:6 }}>Add Amenity</button>
                  </div>
                </div>
                  <div style={{ marginTop:12 }}>
                ...
                <div style={{ display:'flex', gap:8, justifyContent:'flex-end', marginTop:12 }}>
                  ...
                  <button onClick={async () => {
                    ...
                    try {
                      ...
                      // Total = base amount (rent + unit amenities) + custom amenities
                      const customAmenitiesSum = (amenitiesToSend || []).reduce((s, a) => s + (Number(a.cost) || 0), 0);
                      const totalAmount = (Number(createAmount) || 0) + customAmenitiesSum;
                      const payload = {
                        target: createTarget,
                  ...
                  }} style={{ padding:'8px 14px', background:'#d4a017', color:'#1f2937', border:'1px solid #c28e14', borderRadius:6 }}>Create</button>
                </div>
              </div>
            // </div>
          )}
  ...
  // Tenant view
  return (
    <div className="page-container mobile-links-page" style={{ padding: 24, maxWidth: 700 }}>
      <h2 style={{ textAlign: 'center', color: '#1f2937', marginBottom: 28 }}>My Invoices</h2>
      {invoices.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
          {invoices.map(inv => {
            ...
            return (
              <div key={inv._id} style={{ background: '#fffdf8', borderRadius: 14, boxShadow: '0 2px 16px #0001', padding: 24, display: 'flex', flexDirection: 'column', gap: 8, borderLeft: inv.status === 'paid' ? '6px solid #22c55e' : '6px solid #d4a017', position: 'relative' }}>
                <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                  <div style={{ fontWeight: 700, fontSize: 18, color: '#1f2937', marginBottom: 2 }}>{inv.month}</div>
                  {renderSourceBadge(inv)}
                </div>
                <div style={{ fontSize: 15, color: '#64748b', marginBottom: 6 }}>Status: <span style={{ color: inv.status === 'paid' ? '#22c55e' : '#b45309', fontWeight: 600 }}>{inv.status}</span></div>
                {inv.rent != null && (
                  <div style={{ fontSize: 14, marginBottom: 2, color: '#4b5563' }}>Rent: <b>₹{inv.rent}</b></div>
                )}
                {inv.amenities && inv.amenities.length > 0 && (
                  <div style={{ fontSize: 14, marginBottom: 2 }}>
                    <div style={{ fontWeight: 500, color: '#4b5563' }}>Amenities:</div>
                    <ul style={{ margin: '2px 0 4px', paddingLeft: 18 }}>
                      {inv.amenities.map((a, idx) => (<li key={idx} style={{ color: '#374151' }}>{a.name}: ₹{a.cost}</li>))}
                    </ul>
                  </div>
                )}
                <div style={{ fontWeight: 700, fontSize: 16, marginTop: 4, borderTop: '1px solid #f1deab', paddingTop: 6 }}>Total: <span style={{ color: '#b45309' }}>₹{displayedAmount}</span></div>
                <div style={{ fontSize: 15, marginTop: 2 }}>
                  Verification:{" "}
                  <span style={{ color: (inv.action || "pending") === "verified" ? "#22c55e" : "#b45309", fontWeight: 600 }}>
                    {(inv.action || "pending") === "verified" ? "Verified" : "Pending"}
                  </span>
                </div>
                {inv.status === 'paid' ? (
                  ...
                  <button style={{ marginTop: 12, background: '#d4a017', color: '#1f2937', border: '1px solid #c28e14', borderRadius: 6, padding: '8px 28px', fontWeight: 600, fontSize: 15, cursor: 'pointer', alignSelf: 'flex-end' }} onClick={async () => {
                    setPayingInvoice(inv);
                    try {
                      // Fetch owner profile payment details via property owner
                      let propertyId = null;
                      ...
                      if (propertyId) {
                        try {
                          ...
                          const prop = pres.data || null;
                          const ownerId = prop?.owner?._id || prop?.owner;
                          if (ownerId) {
                            const profileRes = await api.get(`/auth/profile/${ownerId}`);
                            const ownerProfile = profileRes.data?.profile || {};
                            setPropertyPaymentInfo({
                              paymentQr: ownerProfile.paymentQr || null,
                              paymentInfo: {
      ...
      <button onClick={() => nav("/dashboard")} style={{ marginTop: 32, background: '#d4a017', color: '#1f2937', border: '1px solid #c28e14', borderRadius: 6, padding: '10px 28px', fontWeight: 600, display: 'block', marginLeft: 'auto', marginRight: 'auto' }}>Back to Dashboard</button>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(0,0,0,0.25)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', overflowY: 'auto', padding: 20, boxSizing: 'border-box' }}>
          <div style={{ background: '#fff', borderRadius: 12, padding: 32, minWidth: 320, maxWidth: 640, width: 'calc(100% - 40px)', maxHeight: '85vh', overflowY: 'auto', boxShadow: '0 4px 32px rgba(0,0,0,0.18)', position: 'relative', boxSizing: 'border-box' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
              <h3 style={{ margin: 0, color: '#1f2937', fontSize: 22 }}>Pay Invoice</h3>
            ...
            <div style={{ marginBottom: 12 }}>
              ...
              <b>Total:</b> <span style={{ color: '#b45309', fontWeight: 600 }}>₹{computeInvoiceAmount(payingInvoice).amt}</span>
            </div>
              ...
              <div style={{ marginTop: 6 }}>
                {propertyPaymentInfo?.paymentInfo ? (
                  <div style={{ fontSize: 14 }}>
                    ...
                    {propertyPaymentInfo.paymentInfo.accountNumber && <div>Account: {propertyPaymentInfo.paymentInfo.accountNumber}</div>}
                    {propertyPaymentInfo.paymentInfo.ifsc && <div>IFSC: {propertyPaymentInfo.paymentInfo.ifsc}</div>}
                    {propertyPaymentInfo.paymentInfo.upiId && <div>UPI: {propertyPaymentInfo.paymentInfo.upiId} </div>}
                  </div>
                ) : (
            ...
            <button style={{ background: '#d4a017', color: '#1f2937', border: '1px solid #c28e14', borderRadius: 6, padding: '10px 28px', fontWeight: 600, cursor: 'pointer', width: '100%' }} disabled={submitLoading || !txnId || !paymentImg} onClick={async () => {
