        ...
        <div className="invoice-card" style={{ background: '#fff', borderRadius: 12, boxShadow: '0 2px 16px #0001', padding: 32 }}>
            ...
            <table className="invoice-table" style={{ width: '100%', borderCollapse: 'collapse', fontSize: 16, background: '#fff' }}>
              ...
              <tbody>
                {filteredInvoices.map((inv, idx) => (
                  <tr key={inv._id} style={{ background: idx % 2 === 0 ? '#f1f5f9' : '#fff', borderBottom: '1px solid #e0e0e0' }}>
          ...
          {showCreateModal && (
            <div style={{ position: 'fixed', top:0,left:0,width:'100vw',height:'100vh',background:'rgba(0,0,0,0.25)',zIndex:1200, display:'flex', alignItems:'center', justifyContent:'center' }}>
              <div style={{ background:'#fff', borderRadius:12, padding:20, width:720, maxWidth:'98%' }}>
                ...
                <div style={{ display:'flex', gap:12, marginTop:12, flexWrap:'wrap', alignItems:'flex-end' }}>
                  <div style={{ minWidth:220 }}>
                    <label>Target</label>
                    <select value={createTarget} onChange={(e) => setCreateTarget(e.target.value)} style={{ width:'100%', padding:8, borderRadius:6 }}>
                  ...
                  <div style={{ flex:1, minWidth:200 }}>
                    ...
                    <select value={selectedProperty} onChange={async (e) => {
                ...
                <div style={{ marginTop:12 }}>
                  ...
                  {createAmenities.map((a, idx) => (
                    <div key={idx} style={{ display:'flex', gap:8, marginTop:6, alignItems:'center' }}>
                      <input placeholder="Name" value={a.name} onChange={e => setCreateAmenities(prev => prev.map((it,i) => i===idx?{...it,name:e.target.value}:it))} style={{ flex:2, minWidth:300, padding:8, borderRadius:6 }} />
                      <input placeholder="Cost" type="number" value={a.cost} onChange={e => setCreateAmenities(prev => prev.map((it,i) => i===idx?{...it,cost:e.target.value}:it))} style={{ width:120, padding:8, borderRadius:6 }} />
                      <button onClick={() => setCreateAmenities(prev => prev.filter((_,i)=>i!==idx))} style={{ background:'#ef4444', color:'#fff', border:'none', padding:'6px 8px', borderRadius:6, width:190 }}>Remove</button>
                    </div>
                  ))}
                  <div style={{ marginTop:8 }}>
                    <button onClick={() => setCreateAmenities(prev => ([...prev, { name:'', cost:0 }]))} style={{ background:'#0ea5e9', color:'#fff', border:'none', padding:'8px 12px', borderRadius:6 }}>Add Amenity</button>
                  </div>
                </div>

                <div style={{ marginTop:12 }}>
                  <label>Description (optional)</label>
                  <textarea value={createDescription} onChange={e => setCreateDescription(e.target.value)} style={{ width:'100%', padding:8, borderRadius:6, minHeight:80 }} />
                </div>
    ...
    <div className="page-container" style={{ padding: 24, maxWidth: 700, margin: '0 auto' }}>
      ...
      {showPayModal && payingInvoice && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(0,0,0,0.25)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: '#fff', borderRadius: 12, padding: 32, minWidth: 340, maxWidth: 400, boxShadow: '0 4px 32px rgba(0,0,0,0.18)', position: 'relative' }}>
            <button onClick={() => { setShowPayModal(false); setTxnId(""); setPaymentImg(null); }} style={{ position: 'absolute', top: 12, right: 16, background: 'none', border: 'none', fontSize: 22, cursor: 'pointer', color: '#888', fontWeight: 700 }}>&times;</button>
            <h3 style={{ marginTop: 0, color: '#0e7490', marginBottom: 16 }}>Pay Invoice</h3>
