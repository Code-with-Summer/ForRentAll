...
const toMoney = (value) => {
  ...
  // Keep amount stable to 2 decimals to avoid float drift.
...
router.get("/all-owner", auth, requireOwnerSubscription, async (req, res) => {
  try {
    // Only allow owner
    ...
    for (const unit of ownerUnits) {
      ...
      if (!invoice) {
        ...
        await Invoice.create({
          unit: unit._id,
          rent: toMoney(Number(unit.rent || 0)),
          amount: toMoney(Number(unit.rent || 0) + Number(amenitiesTotal || 0)),
          month: monthStr,
          status: "pending",
          amenities,
          origin: 'monthly'
...
router.post("/", auth, requireOwnerSubscription, async (req, res) => {
  try {
    if (!req.user || req.user.role !== "owner") return res.status(403).json({ error: "Forbidden" });
    ...
    const createForUnit = async (unitDoc, body) => {
      ...
      // calculate amount
      let amenitiesTotal = 0;
      amenityList = amenityList.map((a) => ({
        ...a,
        cost: toMoney(a?.cost || 0),
      }));
      amenityList.forEach(a => { if (a && typeof a.cost !== "undefined") amenitiesTotal += Number(a.cost || 0); });
      const amount = (typeof body.amount !== "undefined" && body.amount !== null && String(body.amount) !== "")
        ? toMoney(body.amount)
        : toMoney(Number(unitDoc.rent || 0) + amenitiesTotal);
      ...
      const inv = await Invoice.create({
        unit: unitId,
        rent: toMoney(Number(unitDoc.rent || 0)),
        amount,
        month: body.month,
        status: 'due',
    ...
    for (const month of months) {
      if (!invoiceMap[month] || invoiceMap[month].length === 0) {
        ...
        const newInvoice = await Invoice.findOneAndUpdate(
          ...
          { $setOnInsert: { unit: unit._id, amount: toMoney(Number(unit.rent || 0) + Number(amenitiesTotal || 0)), month, status: 'pending', amenities, origin: 'monthly' } },
