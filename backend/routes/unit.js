 ...
 */
router.get("/owner-tenants", auth, requireOwnerSubscription, async (req, res) => {
	try {
...
// Public units listing with optional location filters via property
router.get("/public", async (req, res) => {
	try {
		const activeOwnerIds = await getActiveOwnerIds();
		const { state, city, area } = req.query;
		const propertyFilter = {};
		if (state) propertyFilter.state = state;
		if (city) propertyFilter.city = city;
		if (area) propertyFilter.area = area;
		propertyFilter.adminDeactivated = { $ne: true };
		propertyFilter.subscriptionDeactivated = { $ne: true };
		...
		const unitFilter = {};
		if (propertyIds) {
			unitFilter.property = { $in: propertyIds };
		}

		const units = await Unit.find(unitFilter)
			.populate({
				path: "property",
				select: "name address state city area owner images subscriptionDeactivated",
				populate: { path: "owner", select: "name email phone" }
			})
			.sort({ _id: -1 });

		const visibleUnits = units.filter((u) => {
			const ownerId = u?.property?.owner?._id || u?.property?.owner;
			return ownerId && activeOwnerIds.has(String(ownerId));
		});
...
router.get("/public/:id", async (req, res) => {
	try {
		...
		const unit = await Unit.findById(req.params.id)
			.populate({
				path: "property",
				select: "name address state city area owner images adminDeactivated subscriptionDeactivated",
				populate: { path: "owner", select: "name email phone" }
			})
			.populate("tenant", "name email");

		if (!unit) return res.status(404).json({ error: "Unit not found" });
		if (unit?.property?.adminDeactivated || unit?.property?.subscriptionDeactivated) return res.status(404).json({ error: "Unit not found" });
		const ownerId = unit?.property?.owner?._id || unit?.property?.owner;
		if (!ownerId || !activeOwnerIds.has(String(ownerId))) {
...
router.get("/maintenance/all", auth, requireOwnerSubscription, async (req, res) => {
	try {
...
router.put("/maintenance/:id/status", auth, requireOwnerSubscription, async (req, res) => {
	try {
		if (req.user.role !== "owner") return res.status(403).json({ error: "Forbidden" });
...
router.get("/maintenance/:id", auth, requireOwnerSubscription, async (req, res) => {
	try {
 ...
 */
router.get("/property/:id", auth, requireOwnerSubscription, async (req, res) => {
	try {
 ...
 * Create unit
 */
router.post("/", auth, requireOwnerSubscription, upload.array("photos"), async (req, res) => {
	try {
		...
		const unit = new Unit({
			...
			rent: toMoney(req.body.rent),
			property: req.body.property,
			amenity1: req.body.amenity1,
			amenity1Expense: toMoney(req.body.amenity1Expense),
			amenity2: req.body.amenity2,
			amenity2Expense: toMoney(req.body.amenity2Expense),
			amenity3: req.body.amenity3,
			amenity3Expense: toMoney(req.body.amenity3Expense),
			amenity4: req.body.amenity4,
			amenity4Expense: toMoney(req.body.amenity4Expense),
			photos: photoUrls,
 ...
 */
router.put("/assign-tenant/:id", auth, requireOwnerSubscription, async (req, res) => {
	try {
 ...
 * Update unit
 */
router.put("/:id", auth, requireOwnerSubscription, upload.array("photos"), async (req, res) => {
	try {
		const unit = await Unit.findById(req.params.id).populate("property");
		if (!unit) return res.status(404).json({ error: "Unit not found" });
