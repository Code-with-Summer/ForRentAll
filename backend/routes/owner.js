...
router.put("/profile/:userId", auth, upload.fields([{ name: 'profile_photo', maxCount: 1 }, { name: 'documents', maxCount: 1 }, { name: 'paymentQr', maxCount: 1 }]), async (req, res) => {
  try {
    ...
    if (req.body.bankName !== undefined) profileUpdate.bankName = req.body.bankName;
    if (req.body.accountNumber !== undefined) profileUpdate.accountNumber = req.body.accountNumber;
    if (req.body.ifsc !== undefined) profileUpdate.ifsc = req.body.ifsc;
    if (req.body.upiId !== undefined) profileUpdate.upiId = req.body.upiId;
    if (req.files && req.files.paymentQr && req.files.paymentQr[0]) profileUpdate.paymentQr = req.files.paymentQr[0].filename;
